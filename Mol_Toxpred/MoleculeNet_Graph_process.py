...
from collator_prop import collator, Batch
import argparse
from torch_geometric.loader import DataLoader as pyg_DataLoader
from splitters import scaffold_split
from utils import get_num_task_and_type, get_molecule_repr_MoleculeSTM
...
class MoleculeNetGraphDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        elif self.dataset == 'Ames':
            smiles_list, rdkit_mol_objs, labels = \
                _load_Ames_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'hERG':
            smiles_list, rdkit_mol_objs, labels = \
                _load_hERG_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'DILI':
            smiles_list, rdkit_mol_objs, labels = \
                _load_DILI_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'LD50':
            smiles_list, rdkit_mol_objs, labels = \
                _load_LD50_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)

        elif self.dataset == 'Skin Reaction':
            smiles_list, rdkit_mol_objs, labels = \
                _load_Skin_Reaction_dataset(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)
...
def _load_Ames_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
    # convert 0 to -1
    labels = labels.replace(0, -1)
...
def _load_hERG_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
...
def _load_DILI_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
...
def _load_Skin_Reaction_dataset(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['Drug']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Y']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="LD50")
